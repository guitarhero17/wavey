...
>
  ...
  <div class="flex items-center w-auto m:hidden" id="menu">
    <button class:hidden={ $isAuthenticated } class="p-4 m:py-3 m:px-0 hover:text-waveyYellow text-white m:text-center m:text-xl m:font-bold" on:click={() => handleClickOnLogin()}>
      Log In
    </button>
    <button class:hidden={ !$isAuthenticated } class="p-4 m:py-3 m:px-0 hover:text-waveyYellow text-white m:text-center m:text-xl m:font-bold" on:click={() => handleClickOnLogout()}>
      Log Out
    </button
    >
    <!-- <li>
